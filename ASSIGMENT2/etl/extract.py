...
_DEMO_FINANCE_ASSETS = [
    ("AAPL", "Apple Inc", 224.10, 0.82, 58_200_000),
    ("MSFT", "Microsoft Corp", 431.45, -0.36, 24_100_000),
    ("GOOGL", "Alphabet Inc Class A", 176.30, 1.24, 31_800_000),
    ("AMZN", "Amazon.com Inc", 188.70, 0.45, 42_500_000),
    ("NVDA", "NVIDIA Corp", 138.55, 2.18, 211_000_000),
    ("META", "Meta Platforms Inc", 512.80, -0.74, 16_300_000),
    ("TSLA", "Tesla Inc", 247.90, 1.66, 91_400_000),
    ("JPM", "JPMorgan Chase & Co", 219.25, 0.28, 10_600_000),
    ("JNJ", "Johnson & Johnson", 154.35, -0.19, 7_900_000),
    ("V", "Visa Inc", 291.60, 0.57, 6_200_000),
    ("PG", "Procter & Gamble Co", 168.40, -0.08, 5_700_000),
    ("UNH", "UnitedHealth Group Inc", 586.75, 0.91, 3_800_000),
    ("HD", "Home Depot Inc", 362.15, -1.12, 4_100_000),
    ("MA", "Mastercard Inc", 472.90, 0.63, 2_900_000),
    ("DIS", "Walt Disney Co", 101.25, 1.05, 12_700_000),
    ("NFLX", "Netflix Inc", 706.10, -0.52, 4_300_000),
    ("ADBE", "Adobe Inc", 507.65, 0.34, 2_600_000),
    ("CRM", "Salesforce Inc", 286.35, -0.87, 6_800_000),
    ("PYPL", "PayPal Holdings Inc", 69.80, 1.37, 14_900_000),
    ("BAC", "Bank of America Corp", 41.55, -0.24, 38_600_000),
]


def _demo_realtime_finance(symbols: list[str] | None = None) -> pd.DataFrame:
    """Return deterministic demo finance rows when RapidAPI is unavailable."""
    import datetime

    selected = _DEMO_FINANCE_ASSETS
    if symbols:
        wanted = {s.upper() for s in symbols}
        selected = [asset for asset in selected if asset[0] in wanted]

    current_year = datetime.datetime.now().year
    years = list(range(current_year - 4, current_year + 1))
    rows = []

    for idx, (_symbol, company_name, current_price, change_pct, volume) in enumerate(selected):
        for offset, year in enumerate(years):
            distance_from_current = offset - (len(years) - 1)
            trend_factor = 1 + (distance_from_current * 0.04) + ((idx % 5) - 2) * 0.003
            price = round(current_price * trend_factor, 2)
            year_change_pct = round(change_pct + distance_from_current * 0.18, 2)
            previous_close = round(price / (1 + year_change_pct / 100), 2)
            price_change = round(price - previous_close, 2)
            open_price = round(previous_close * (1 + (year_change_pct / 100) * 0.35), 2)
            day_high = round(max(price, open_price, previous_close) * 1.012, 2)
            day_low = round(min(price, open_price, previous_close) * 0.988, 2)
            year_volume = int(volume * (0.88 + offset * 0.03 + (idx % 4) * 0.01))
            ...
            values = {
                ...
                "day_low_usd": day_low,
                "trading_volume": year_volume,
                "previous_close_usd": previous_close,
                "price_change_usd": price_change,
                "price_change_pct": year_change_pct,
            }

            for indicator_label, value in values.items():
                rows.append({
                    "country_code": "",
                    ...
                    "year": year,
                    "value": float(value),
                    "source": "Offline Demo Finance",
                })

    df = pd.DataFrame(rows)
    print(f"  [demo] Offline finance data: {len(df):,} rows")
    return df[["country_code", "country_name", "indicator", "year", "value", "source"]]
...
def extract_realtime_finance(symbols: list[str] | None = None) -> pd.DataFrame:
    ...
    if not api_key:
        print("  [fallback] RapidAPI: RAPIDAPI_KEY not set; using offline demo finance data")
        return _demo_realtime_finance(symbols)
    ...
    if not frames:
        print("  [fallback] No data fetched from RapidAPI; using offline demo finance data")
        return _demo_realtime_finance(symbols)
