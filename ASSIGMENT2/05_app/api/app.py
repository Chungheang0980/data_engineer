...
def list_assets():
    ...
    if not _tables_exist("gold.dim_asset"):
        return []
...
def list_prices(sector: Optional[str] = None):
    ...
    """
    if not _tables_exist("gold.dim_asset", "gold.fact_prices"):
        return []
...
def get_price(asset_name: str):
    ...
    if not _tables_exist("gold.dim_asset", "gold.fact_prices"):
        raise HTTPException(404, "Warehouse has no price data yet")
...
def get_predictions(asset_name: str, model: Optional[str] = None):
    ...
    """
    if not _tables_exist("gold.dim_asset", "gold.fact_predictions"):
